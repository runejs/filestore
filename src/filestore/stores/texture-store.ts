...
export class Texture {
    ...
    public generatePixels(spriteStore: SpriteStore): boolean {
        ...
        for (let i = 0; i < this.spriteIds.length; i++) {
            ...
            for (let j = 0; j < spritePalette.length; j++) {
                spritePalette[j] = ColorUtils.method707(
                    spritePalette[j],
        ...
        if (this.pixels != null) {
            if (this.direction === 1 || this.direction === 3) {
                ...
                for (let i = 0; i < colorCount; i++) {
                    ...
                    Texture.pixelsBuffer[
                        i + colorCount + colorCount + colorCount
                    ] =
                        this.pixels[
                            i_4_ + colorCount + colorCount + colorCount
                        ];
                }
            ...
            if (this.direction === 2 || this.direction === 4) {
                ...
                for (let x = 0; x < colorCount; x += size) {
                    for (let y = 0; y < size; y++) {
                        ...
                        Texture.pixelsBuffer[
                            i_10_ + colorCount + colorCount + colorCount
                        ] =
                            this.pixels[
                                i_11_ + colorCount + colorCount + colorCount
                            ];
                    }
                }
                const is = this.pixels;
