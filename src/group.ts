...
export class Group extends IndexedFile<GroupIndexEntity> {
    ...
    readonly files: Map<string, FlatFile> = new Map<string, FlatFile>();
    readonly fileSizes: Map<string, number> = new Map<string, number>();
    ...
    constructor(index: GroupIndexEntity, breadcrumb?: Partial<FileBreadcrumb>) {
    ...
    override decode(): ByteBuffer | null {
        this.unpack();
        this.decompress();
    ...
    override encode(): ByteBuffer | null {
        // Single-file group
        if (this._fileCount === 1) {
    ...
    override async read(compress: boolean = false, readDiskFiles: boolean = true): Promise<ByteBuffer | null> {
        if (!this.index) {
    ...
    has(fileIndex: string | number): boolean {
        return this.files.has(String(fileIndex));
    }

    get(fileIndex: string | number): FlatFile | null {
        return this.files.get(String(fileIndex)) ?? null;
    }

    set(fileIndex: string | number, file: FlatFile): void {
