...
export class Archive extends IndexedFile<ArchiveIndexEntity> {
    ...
    constructor(index: ArchiveIndexEntity, config: ArchiveConfig, breadcrumb?: Partial<FileBreadcrumb>) {
    ...
    override decode(decodeGroups: boolean = true): ByteBuffer | null {
    ...
    override encode(encodeGroups: boolean = true): ByteBuffer | null {
        if (this.numericKey === 255) {
    ...
    override compress(compressGroups: boolean = true): ByteBuffer | null {
        if (compressGroups) {
    ...
    override async read(compress: boolean = false, readDiskFiles: boolean = true): Promise<ByteBuffer> {
    ...
    async saveIndexData(saveGroups: boolean = true, saveFiles: boolean = true): Promise<void> {
        if (!this.groups.size) {
            return;
        }
    ...
    async saveGroupIndexes(saveFlatFiles: boolean = true): Promise<void> {
    ...
    async saveFlatFileIndexes(): Promise<void> {
    ...
    has(groupKey: number): boolean;
    has(groupKey: string | number): boolean;
    has(groupKey: string | number): boolean {
        return this.groups.has(String(groupKey));
    }

    get(groupKey: string): Group | null;
    get(groupKey: number): Group | null;
    get(groupKey: string | number): Group | null;
    get(groupKey: string | number): Group | null {
        return this.groups.get(String(groupKey)) ?? null;
    }

    set(groupKey: string, group: Group): void;
    set(groupKey: number, group: Group): void;
    set(groupKey: string | number, group: Group): void;
    set(groupKey: string | number, group: Group): void {
        this.groups.set(String(groupKey), group);
    }

    find(groupName: string): Archive | Group | FlatFile | null {
        const children = Array.from(this.groups.values());
        return children.find(child => child?.name === groupName) ?? null;
    }

    incrementMissingEncryptionKeys(): void {
        this._missingEncryptionKeys++;
    }

    get missingEncryptionKeys(): number {
    ...
    override get outputPath(): string {
