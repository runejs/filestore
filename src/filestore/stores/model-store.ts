...
export class RsModel {
            ...
            if (textureId !== -1) {
                ...
                if (texturedFaceTypeIndex === -1) {
                    ...
                    if (this.texturedFaceTypes != null) {
                        texturedFaceType =
                            this.texturedFaceTypes[texturedFaceTypeIndex];
                    }
                    ...
                    if (texturedFaceType === 0) {
                        ...
                        const texturedFaceIndexA =
                            this.texturedFaceIndicesA[texturedFaceTypeIndex];
                        const texturedFaceIndexB =
                            this.texturedFaceIndicesB[texturedFaceTypeIndex];
                        const texturedFaceIndexC =
                            this.texturedFaceIndicesC[texturedFaceTypeIndex];
                        ...
                        const f_882_ =
                            this.verticesX[texturedFaceIndexB] - vertexX;
                        const f_883_ =
                            this.verticesY[texturedFaceIndexB] - vertexY;
                        const f_884_ =
                            this.verticesZ[texturedFaceIndexB] - vertexZ;
                        const f_885_ =
                            this.verticesX[texturedFaceIndexC] - vertexX;
                        const f_886_ =
                            this.verticesY[texturedFaceIndexC] - vertexY;
                        const f_887_ =
                            this.verticesZ[texturedFaceIndexC] - vertexZ;
    ...
    public applyLighting(
        ...
        if (this.vertexNormals === null) {
            ...
            this.vertexNormals = initializeVertexNormals.map(
                () => new VertexNormal(),
            );
        }
        for (let i_52_ = 0; i_52_ < this.faceCount; i_52_++) {
            ...
            let i_65_ = ~~Math.sqrt(
                i_62_ * i_62_ + i_63_ * i_63_ + i_64_ * i_64_,
            );
            if (i_65_ <= 0) {
                i_65_ = 1;
            }
            ...
            if (this.faceTypes == null || (this.faceTypes[i_52_] & 0x1) === 0) {
                ...
                const i_66_ =
                    ...
                    (arg2 * i_62_ + arg3 * i_63_ + arg4 * i_64_) /
                        (i_50_ + i_50_ / 2);
                this.faceColorsX[i_52_] = ColorUtils.method816(
                    this.faceColors[i_52_],
                    i_66_,
                    this.faceTypes[i_52_],
                );
            }
        }
        if (applyShading) {
            ...
            this.vertexNormalOffsets = new Array<VertexNormal>(
                this.vertexCount,
            );
            for (let i_67_ = 0; i_67_ < this.vertexCount; i_67_++) {
                ...
                const normalOffset = (this.vertexNormalOffsets[i_67_] =
                    new VertexNormal());
    ...
    public applyShading(
        ...
        for (let i = 0; i < this.faceCount; i++) {
            ...
            if (this.faceTypes == null) {
                ...
                let i_76_ = ~~(
                    arg0 +
                    (arg2 * normal.x + arg3 * normal.y + arg4 * normal.z) /
                        (arg1 * normal.magnitude)
                );
                ...
                this.faceColorsX[i] = ColorUtils.method816(
                    faceColor,
                    i_79_,
                    faceType,
                );
                ...
                this.faceColorsY[i] = ColorUtils.method816(
                    faceColor,
                    i_79_,
                    faceType,
                );
                ...
                this.faceColorsZ[i] = ColorUtils.method816(
                    faceColor,
                    i_79_,
                    faceType,
                );
...
export class ColorUtils {
    private static readonly UNKNOWN_COLOR_TABLE =
        ColorUtils.initUnknownColorTable();
...
export class ModelStore {
    ...
    public getModel(id: number): RsModel | null {
        ...
        const vertexDirectionOffsetBuffer = new ByteBuffer(buffer);
        const xDataOffsetBuffer = new ByteBuffer(buffer);
        const yDataOffsetBuffer = new ByteBuffer(buffer);
        const zDataOffsetBuffer = new ByteBuffer(buffer);
        const vertexSkinOffsetBuffer = new ByteBuffer(buffer);
        vertexDirectionOffsetBuffer.readerIndex = buffer.length - 18;
        rsModel.vertexCount = vertexDirectionOffsetBuffer.get(
            'SHORT',
            'UNSIGNED',
        );
        rsModel.faceCount = vertexDirectionOffsetBuffer.get(
            'SHORT',
            'UNSIGNED',
        );
        rsModel.texturedFaceCount = vertexDirectionOffsetBuffer.get(
            'BYTE',
            'UNSIGNED',
        );
        const hasFaceTypes = vertexDirectionOffsetBuffer.get(
            'BYTE',
            'UNSIGNED',
        );
        const modelPriority = vertexDirectionOffsetBuffer.get(
            'BYTE',
            'UNSIGNED',
        );
        const hasFaceAlphas = vertexDirectionOffsetBuffer.get(
            'BYTE',
            'UNSIGNED',
        );
        const hasFaceSkins = vertexDirectionOffsetBuffer.get(
            'BYTE',
            'UNSIGNED',
        );
        const hasVertexSkins = vertexDirectionOffsetBuffer.get(
            'BYTE',
            'UNSIGNED',
        );
        const verticesXSubOffset = vertexDirectionOffsetBuffer.get(
            'SHORT',
            'UNSIGNED',
        );
        const verticesYSubOffset = vertexDirectionOffsetBuffer.get(
            'SHORT',
            'UNSIGNED',
        );
        const verticesZSubOffset = vertexDirectionOffsetBuffer.get(
            'SHORT',
            'UNSIGNED',
        );
        const faceIndicesSubOffset = vertexDirectionOffsetBuffer.get(
            'SHORT',
            'UNSIGNED',
        );
        let offset = 0;
        ...
        if (rsModel.texturedFaceCount > 0) {
            rsModel.texturedFaceTypes = new Uint8Array(
                rsModel.texturedFaceCount,
            );
            rsModel.texturedFaceIndicesA = new Uint16Array(
                rsModel.texturedFaceCount,
            );
            rsModel.texturedFaceIndicesB = new Uint16Array(
                rsModel.texturedFaceCount,
            );
            rsModel.texturedFaceIndicesC = new Uint16Array(
                rsModel.texturedFaceCount,
            );
        }
        if (hasVertexSkins === 1) {
        ...
        for (let i = 0; i < rsModel.vertexCount; i++) {
            ...
            if ((mask & 0x1) !== 0) {
                try {
                    ...
                    logger.warn(
                        'Tried to read out of range xOffset for object',
                        id,
                    );
                }
            }
            ...
            if ((mask & 0x2) !== 0) {
                try {
                    ...
                    logger.warn(
                        'Tried to read out of range yOffset for object',
                        id,
                    );
                }
            }
            ...
            if ((mask & 0x4) !== 0) {
                try {
                    ...
                    logger.warn(
                        'Tried to read out of range zOffset for object',
                        id,
                    );
                }
            }
            ...
            if (hasVertexSkins === 1) {
                rsModel.vertexSkins[i] = vertexSkinOffsetBuffer.get(
                    'BYTE',
                    'UNSIGNED',
                );
            }
        ...
        for (let i = 0; i < rsModel.faceCount; i++) {
            rsModel.faceColors[i] = vertexDirectionOffsetBuffer.get(
                'SHORT',
                'UNSIGNED',
            );
            if (hasFaceTypes === 1) {
            ...
            if (modelPriority === 255) {
                rsModel.facePriorities[i] = yDataOffsetBuffer.get(
                    'BYTE',
                    'UNSIGNED',
                );
            }
            if (hasFaceAlphas === 1) {
                rsModel.faceAlphas[i] = zDataOffsetBuffer.get(
                    'BYTE',
                    'UNSIGNED',
                );
            }
            if (hasFaceSkins === 1) {
                rsModel.faceSkins[i] = vertexSkinOffsetBuffer.get(
                    'BYTE',
                    'UNSIGNED',
                );
            }
        ...
        for (let i = 0; i < rsModel.faceCount; i++) {
            ...
            switch (type) {
                case 1:
                    lastA =
                        vertexDirectionOffsetBuffer.get(
                            'SMART_SHORT',
                            'UNSIGNED',
                        ) + accumulator;
                    accumulator = lastA;
        ...
        for (let i = 0; i < rsModel.texturedFaceCount; i++) {
            rsModel.texturedFaceIndicesA[i] = vertexDirectionOffsetBuffer.get(
                'SHORT',
                'UNSIGNED',
            );
            rsModel.texturedFaceIndicesB[i] = vertexDirectionOffsetBuffer.get(
                'SHORT',
                'UNSIGNED',
            );
            rsModel.texturedFaceIndicesC[i] = vertexDirectionOffsetBuffer.get(
                'SHORT',
                'UNSIGNED',
            );
        }
        if (rsModel.texturedFaceTypeIndices != null) {
            ...
            for (let face = 0; face < rsModel.faceCount; face++) {
                const texturedFaceTypeIndex =
                    rsModel.texturedFaceTypeIndices[face] & 0xff;
                if (texturedFaceTypeIndex !== 255) {
                    if (
                        (rsModel.texturedFaceIndicesA[texturedFaceTypeIndex] &
                            0xffff) ===
                            rsModel.faceIndicesA[face] &&
                        (rsModel.texturedFaceIndicesB[texturedFaceTypeIndex] &
                            0xffff) ===
                            rsModel.faceIndicesB[face] &&
                        (rsModel.texturedFaceIndicesC[texturedFaceTypeIndex] &
                            0xffff) ===
                            rsModel.faceIndicesC[face]
                    ) {
